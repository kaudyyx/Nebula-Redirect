    ...
    namespace Safety
    {
        ...
        inline static auto PrintStack(CONTEXT* ctx) -> void
        {
            ...
            for (ULONG frame = 0;; frame++)
            {
                ...
                lstrcpyA(module, "");
                // When using GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS the second parameter
                // must be an address (LPCVOID). Passing it as a wchar_t* caused a
                // const wchar_t* -> LPCSTR mismatch on some project settings.
                GetModuleHandleExA(
                    GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                    reinterpret_cast<LPCSTR>(stack.AddrPC.Offset),
                    &hModule
                );
