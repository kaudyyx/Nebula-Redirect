#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: SR_Core

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "SrirachaRanch_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass SR_Core.SR_Core_C
// 0x0010 (0x0390 - 0x0380)
class USR_Core_C final : public UStreamingRadioPlayerComponent
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame;                                    // 0x0380(0x0008)(ZeroConstructor, Transient, DuplicateTransient)
	bool                                          bMixIsActive;                                      // 0x0388(0x0001)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, IsPlainOldData, NoDestructor)
	bool                                          bIsADS;                                            // 0x0389(0x0001)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, IsPlainOldData, NoDestructor)
	bool                                          bIsInsideVehicle;                                  // 0x038A(0x0001)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, IsPlainOldData, NoDestructor)

public:
	void ExecuteUbergraph_SR_Core(int32 EntryPoint);
	void OnPawnExitSeat(const TScriptInterface<class IFortVehicleInterface>& Vehicle, class AFortPawn* PlayerPawn, int32 SeatIndex);
	void OnPawnEnterSeat(const TScriptInterface<class IFortVehicleInterface>& Vehicle, class AFortPawn* PlayerPawn, int32 SeatIndex);
	void HandleSeatStateUpdated(class AFortPawn* Pawn, bool Entering, int32 SeatIdx);
	void HandleWeaponStateUpdated(class AFortWeapon* NewWeapon, class AFortWeapon* PrevWeapon);
	void HandleADSStateUpdated(bool Is_Targeting);
	void HandleOnSourcePlayed(class UStreamingRadioPlayerComponent* Component, class URadioContentSourceItemDefinition* AffectedSource);
	void UpdateMixState();
	void GetIsMixAllowed(bool* MixAllowed);

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"SR_Core_C">();
	}
	static class USR_Core_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<USR_Core_C>();
	}
};
static_assert(alignof(USR_Core_C) == 0x000008, "Wrong alignment on USR_Core_C");
static_assert(sizeof(USR_Core_C) == 0x000390, "Wrong size on USR_Core_C");
static_assert(offsetof(USR_Core_C, UberGraphFrame) == 0x000380, "Member 'USR_Core_C::UberGraphFrame' has a wrong offset!");
static_assert(offsetof(USR_Core_C, bMixIsActive) == 0x000388, "Member 'USR_Core_C::bMixIsActive' has a wrong offset!");
static_assert(offsetof(USR_Core_C, bIsADS) == 0x000389, "Member 'USR_Core_C::bIsADS' has a wrong offset!");
static_assert(offsetof(USR_Core_C, bIsInsideVehicle) == 0x00038A, "Member 'USR_Core_C::bIsInsideVehicle' has a wrong offset!");

}

