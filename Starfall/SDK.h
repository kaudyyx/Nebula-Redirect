...
namespace USDK
{
	...
	private:
		...
		{
			if ( !IsValidIndex( Index ) ) throw out_of_range( "Index was out of range!" );
		}

		inline _Et& GetUnsafe( int32 Index )
		{
			return Data[Index];
		}
		inline const _Et& GetUnsafe( int32 Index ) const
		{
			return Data[Index];
		}
	...
	public:
		...
		inline void Clear( )
		{
			...
			if ( Data && OldNum > 0 )
				__stosb( (PBYTE) Data, 0, OldNum * sizeof( _Et ) );
		}
	...
	public:
		inline _Et& operator[]( int32 Index )
		{
			VerifyIndex( Index ); return Data[Index];
		}
		inline const _Et& operator[]( int32 Index ) const
		{
			VerifyIndex( Index ); return Data[Index];
		}

		inline const _Et* GetPtr( int32 Index, int32 Size ) const
		{
			VerifyIndex( Index ); return (_Et*) ( __int64( Data ) + ( Index * Size ) );
		}
		inline const _Et& Get( int32 Index, int32 Size ) const
		{
			VerifyIndex( Index ); return *GetPtr( Index, Size );
		}
	...
	class SDKFString : public TArray<wchar_t>
	{
	public:
		using TArray::TArray;

		SDKFString( const wchar_t* Str )
		{
	...
	public:
		inline bool operator==( const SDKFString& Other ) const
		{
	...
	public:
		...
		int32 Number;
		static inline void( *AppendString )( const FName*, SDKFString& ) = ( [ ] ( )
		{
			...
			for ( auto& sig : sigs )
			{
				...
				if ( Scanner.Get( ) )
				{
					...
					return ( void( * )( const FName*, SDKFString& ) )Scanner.RelativeOffset( (uint32_t) pb.size( ) ).Get( );
				}
			}
			return ( void( * )( const FName*, SDKFString& ) )nullptr;
		} )( );

		string ToString( ) const
		{
			thread_local SDKFString TempString( 1024 );
	...
	public:
		...
		uint32 GetOffset( const char* Name ) const
		{
			...
			if ( !Prop ) return static_cast<uint32>( -1 );
		...
		T& Get( ) const
		{
			static auto Off = GetOffset( reinterpret_cast<const char*>( &Name ) );
			if ( Off == static_cast<uint32>( -1 ) )
			{
		...
		{
			static auto OffsetOff = FNVer >= 12.10 && FNVer < 20 ? 0x4c : 0x44;
			static auto Prop = GetProperty( reinterpret_cast<const char*>( &Name ) );
		...
		bool Has( ) const
		{
			return GetOffset( reinterpret_cast<const char*>( &Name ) ) != static_cast<uint32>( -1 );
		}
		...
		{
			static auto Off = GetOffset( reinterpret_cast<const char*>( &Name ) );
			if ( Off == static_cast<uint32>( -1 ) ) return nullptr;
		...
		{
			static auto Function = GetFunction( reinterpret_cast<const char*>( &Name ) );
	...
	public:
		...
		void* CreateParams( std::vector<ParamPair> InputParams )
		{
			...
			for ( auto& _Pa : InputParams )
			{
				...
				int i = 0;
				uint32 Size = 0;
				for ( auto& _Mp : ParamsInfo.NameOffsetMap )
				{
					if ( _Mp.Name == _Pa._Fi )
					{
						FoundParam = _Mp;
						Size = i == ParamsInfo.NameOffsetMap.size( ) - 1 ? ParamsInfo.Size - _Mp.Offset : ParamsInfo.NameOffsetMap[i + 1].Offset - _Mp.Offset;
						break;
					}
					i++;
				}
		...
		_Rt* GetValueFromParams( void* ParamsPtr, const char* Name )
		{
			auto ParamsInfo = GetParams( );
			...
			for ( auto& _Mp : ParamsInfo.NameOffsetMap )
			{
				if ( _Mp.Name == Name )
				{
					return GetPtrFromOffset<_Rt>( ParamsPtr, _Mp.Offset );
				}
			}
	...
	public:
		...
		static inline TUObjectArrayUnchunked* GObjectsUnchunked = ( [ ] ( )
		{
			if ( FNVer < 5.00 )
			{
				...
				if ( !Addr.Get( ) )
				{
					Addr = Memcury::Scanner::FindPattern( "48 8B 05 ? ? ? ? 48 8D 1C C8 81 4B ? ? ? ? ? 49 63 76 30" );
				}
	...
	__forceinline void Hook( void* detour, T& og = nullptrForHook )
	{
		...
		int idx = DefaultObj->GetFunction( reinterpret_cast<const char*>( &FuncName ) )->GetVTableIndex( );
		if ( !is_same_v<T, void*> )
			og = (T) VTable[idx];
	...
	{
		static auto Class = TUObjectArray::FindObject<UClass>( reinterpret_cast<const char*>( &Str ) );
		return GetAll( Class );
	}
	...
	AActor* SpawnActor( const UClass* Class, FVector Loc, FRotator Rot = {}, AActor* Owner = nullptr )
	{
		...
		Params.CollisionHandlingOverride = 2;
		Params.Owner = Owner;
	...
	{
	public:
		class SDKFString InString;
		class FName ReturnValue;
	};

	FName Conv_StringToName( SDKFString Str )
	{
	...
	public:
		...
		class SDKFString SubPathString;
	};
